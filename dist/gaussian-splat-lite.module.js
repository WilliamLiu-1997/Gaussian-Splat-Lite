...
import { Loader, Quaternion, Vector3, Color, Matrix4 } from "three";
...
_SplatAccumulator.viewDirUniform = new DynoVec3({ value: new THREE.Vector3() });
...
_SplatAccumulator.fullScreenQuad = new FullScreenQuad(
  new THREE.RawShaderMaterial({ visible: false })
);
...
const _SparkRenderer = class _SparkRenderer extends THREE.Mesh {
  ...
  onBeforeRender(renderer, scene, camera) {
    ...
    if (currentRenderTarget) {
      ...
      if (isXRRenderTarget && spark.renderSize.x === 1 && spark.renderSize.y === 1) {
    ...
    const outputColorSpace = currentRenderTarget === null ? renderer.outputColorSpace : isXRRenderTarget ? currentRenderTarget.texture.colorSpace : THREE.ColorManagement.workingColorSpace;
    this.uniforms.encodeLinear.value = outputColorSpace !== THREE.SRGBColorSpace;
  ...
  async renderCubeMap({
    ...
    if (!_SparkRenderer.cubeRender || _SparkRenderer.cubeRender.target.width !== size || _SparkRenderer.cubeRender.near !== near || _SparkRenderer.cubeRender.far !== far) {
      ...
      const cubeCamera2 = new THREE.CubeCamera(near, far, target2);
  ...
  async renderEnvMap({
    ...
    if (!_SparkRenderer.pmrem) {
      _SparkRenderer.pmrem = new THREE.PMREMGenerator(this.renderer);
    }
...
const DEFAULT_CONTROLLER_GETMOVE = (gamepads, sparkXr) => {
  var _a, _b, _c, _d;
  return gamepads.leftIsHand ? new THREE.Vector3() : new THREE.Vector3(
...
const DEFAULT_CONTROLLER_GETROTATE = (gamepads, sparkXr) => {
  var _a;
  return gamepads.rightIsHand ? new THREE.Vector3() : new THREE.Vector3(((_a = gamepads.right) == null ? void 0 : _a.axes[2]) ?? 0, 0, 0);
};
...
class JointSplats extends SplatMesh {
  constructor(hand) {
    super({});
    this.scratchCenter = new THREE.Vector3();
    this.scratchQuat = new THREE.Quaternion(0, 0, 0, 1);
    this.scratchScales = new THREE.Vector3().setScalar(0.01);
    this.scratchColor = new THREE.Color(1, 1, 1);
    this.hand = hand;
  }
  updateJoints(joints) {
    this.visible = false;
...
class FpsMovement {
  constructor({
    ...
    this.enable = true;
    this.extraMove = new THREE.Vector3();
  ...
  update(deltaTime, control, camera) {
    ...
    if (this.dualPress && this.rotating && this.sliding) {
      ...
      if (coincidence >= 0.2) {
        ...
        const theCamera = camera ?? (control instanceof THREE.Camera ? control : void 0);
        if (theCamera) {
      ...
      if (this.sliding && !this.dualPress) {
        ...
        if (this.sliding || this.rotating) {
          const point = ((_a = this.lastDown) == null ? void 0 : _a.last) ?? new THREE.Vector2();
          const theCamera = camera ?? (control instanceof THREE.Camera ? control : void 0);
          if (theCamera) {
            const ndcPoint = this.pressMoveCenter ? new THREE.Vector2(0, 0) : new THREE.Vector2(
...
class SparkPortals {
  constructor(options) {
    this.portalPairs = [];
    this.lastCameraWorld = new THREE.Vector3().setScalar(Number.NaN);
    this.scratch = {
      quat: new THREE.Quaternion(),
      scale: new THREE.Vector3(),
...
export {
  ...
  DEFAULT_CONTROLLER_GETFAST,
  DEFAULT_CONTROLLER_GETMOVE,
  DEFAULT_CONTROLLER_GETROTATE,
  DEFAULT_CONTROLLER_GETSLOW,
  DEFAULT_CONTROLLER_MOVE_HEADING,
  DEFAULT_CONTROLLER_MOVE_SPEED,
  DEFAULT_CONTROLLER_ROLL_SPEED,
  DEFAULT_CONTROLLER_ROTATE_SPEED,
  DEFAULT_CONTROLLER_SLOW_MULTIPLIER,
...
//# sourceMappingURL=gaussian-splat-lite.module.js.map
