...
class SplatMesh extends THREE.Object3D {
  constructor(options = {}) {
    ...
    this.lastViewOrigin = new THREE.Vector3().setScalar(Number.NaN);
...
const _GaussianSplatRenderer = class _GaussianSplatRenderer extends THREE.Mesh {
  constructor(options) {
    if (!options) {
  ...
  async renderEnvMap({
    ...
    if (!_GaussianSplatRenderer.pmrem) {
      _GaussianSplatRenderer.pmrem = new THREE.PMREMGenerator(this.renderer);
    }
