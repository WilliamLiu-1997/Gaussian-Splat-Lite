...
import { Loader as PA } from "three";
import { FullScreenQuad as jA } from "three/addons/postprocessing/Pass.js";
function zA(C, A) {
  const I = C.elements, g = A.x, B = A.y, Q = A.z, E = I[0] * g + I[4] * B + I[8] * Q + I[12], i = I[1] * g + I[5] * B + I[9] * Q + I[13], D = I[2] * g + I[6] * B + I[10] * Q + I[14];
  return I[12] = E, I[13] = i, I[14] = D, C;
}
class OA {
  constructor() {
    this.entries = /* @__PURE__ */ new Map(), this.freeMeshIds = [], this.nextMeshId = 0;
  }
  allocateMeshId() {
    const A = this.freeMeshIds.pop();
  ...
  dispose() {
    this.entries.clear(), this.freeMeshIds.length = 0, this.nextMeshId = 0;
  }
  prepare(A) {
    const I = new Uint32Array(A.mapping.length), g = new Uint32Array(A.mapping.length), B = new Uint32Array(A.mapping.length), Q = new Float64Array(A.mapping.length * 3), E = new Set(this.entries.keys()), i = [];
    let D = 0;
    A.mapping.forEach(
      ({ node: c, base: e, count: t, sortVersion: h }, w) => {
        E.delete(c);
        let G = this.entries.get(c);
        G || (G = {
          meshId: this.allocateMeshId(),
          sortVersion: -1
        }, this.entries.set(c, G)), I[w] = G.meshId, g[w] = e, B[w] = t;
        const N = c.matrixWorld.elements, R = w * 3;
        Q[R] = N[12], Q[R + 1] = N[13], Q[R + 2] = N[14], G.sortVersion !== h && (i.push({
          node: c,
          entry: G,
          count: t,
          rangeIndex: w,
    ...
    const s = new Uint32Array(i.length), a = new Float32Array(D * 3);
    a.fill(Number.NaN);
    let y = 0;
    return i.forEach(({ node: c, count: e, rangeIndex: t }, h) => {
      var G;
      s[h] = t;
      const w = c.matrixWorld.elements;
      (G = c.splats) == null || G.forEachCenter((N, R, S, J) => {
        if (N >= e || Number.isNaN(R)) return;
        const k = (y + N) * 3;
        a[k] = w[0] * R + w[4] * S + w[8] * J, a[k + 1] = w[1] * R + w[5] * S + w[9] * J, a[k + 2] = w[2] * R + w[6] * S + w[10] * J;
      }), y += e;
    }), {
      payload: {
        updateRangeIndices: s,
        updateCenters: a,
        rangeMeshIds: I,
        rangeBases: g,
        rangeCounts: B,
      ...
      commit: () => {
        for (const { entry: c, sortVersion: e } of i)
          c.sortVersion = e;
        for (const c of E) {
          const e = this.entries.get(c);
          e && (this.entries.delete(c), this.freeMeshIds.push(e.meshId));
        }
      }
    };
  }
}
var _A = /* @__PURE__ */ ((C) => (C.ALL = "all", C.PLANE = "plane", C.SPHERE = "sphere", C.BOX = "box", C.ELLIPSOID = "ellipsoid", C.CYLINDER = "cylinder", C.CAPSULE = "capsule", C.INFINITE_CONE = "infinite_cone", C))(_A || {}), $A = /* @__PURE__ */ ((C) => (C.MULTIPLY = "multiply", C.SET_RGB = "set_rgb", C.ADD_RGBA = "add_rgba", C))($A || {});
class AI extends o.Object3D {
  constructor(A = {}) {
    super(), this.type = A.type ?? "sphere", this.invert = A.invert ?? !1, this.opacity = A.opacity ?? 1, this.color = A.color ?? new o.Color(1, 1, 1), this.radius = A.radius ?? 0;
  }
}
const iA = class iA extends o.Object3D {
  constructor(A = {}) {
    super(), this.rgbaBlendMode = A.rgbaBlendMode ?? "multiply", this.sdfSmooth = A.sdfSmooth ?? 0, this.softEdge = A.softEdge ?? 0, this.invert = A.invert ?? !1, this.sdfs = A.sdfs ?? null, this.ordering = iA.nextOrdering++, this.name = A.name ?? `Edit ${this.ordering}`;
  }
  addSdf(A) {
    this.sdfs ?? (this.sdfs = []), this.sdfs.includes(A) || this.sdfs.push(A);
  }
  removeSdf(A) {
    this.sdfs && (this.sdfs = this.sdfs.filter((I) => I !== A));
  }
};
iA.nextOrdering = 1;
let BA = iA;
const b = 5, NA = 16, eA = new Float32Array(1), FA = new Uint32Array(eA.buffer), GA = class GA {
  constructor({ maxSdfs: A = 0, maxEdits: I = 0 } = {}) {
    this.numSdfs = 0, this.numEdits = 0, this.maxSdfs = Math.max(NA, A), this.sdfData = new Uint32Array(this.maxSdfs * b * 4), this.sdfFloatData = new Float32Array(this.sdfData.buffer), this.sdfTexture = m(this.sdfData, b, this.maxSdfs), this.maxEdits = Math.max(NA, I), this.editData = new Uint32Array(this.maxEdits * 4), this.editFloatData = new Float32Array(this.editData.buffer), this.editTexture = m(this.editData, 1, this.maxEdits);
  }
  dispose() {
    this.sdfTexture.dispose(), this.editTexture.dispose();
  }
  update(A, I) {
    const g = A.reduce(
      (e, t) => e + t.sdfs.length,
    ...
    let B = this.ensureCapacity(g, A.length);
    (this.numSdfs !== g || this.numEdits !== A.length) && (this.numSdfs = g, this.numEdits = A.length, B = !0);
    const Q = new o.Vector3(), E = new o.Quaternion(), i = new o.Vector3(), D = new o.Vector4(), s = new o.Matrix4();
    let a = 0, y = !1, c = !1;
    return A.forEach(({ edit: e, sdfs: t }, h) => {
      c = this.encodeEdit(h, e, a, t.length) || c;
      for (const w of t) {
        D.set(w.scale.x, w.scale.y, w.scale.z, w.radius);
        const G = w.scale.clone();
        try {
          w.scale.setScalar(1), w.updateWorldMatrix(!0, !1), s.copy(w.matrixWorld).invert(), I && zA(s, I), s.decompose(Q, E, i);
        } finally {
          w.scale.copy(G), w.updateWorldMatrix(!0, !1);
        }
        y = this.encodeSdf(
          a,
          w,
    ...
    }), y && (this.sdfTexture.needsUpdate = !0), c && (this.editTexture.needsUpdate = !0), { updated: B || y || c };
  }
  ensureCapacity(A, I) {
    let g = !1;
    return A > this.maxSdfs && (this.maxSdfs = Math.max(A, this.maxSdfs * 2), this.sdfTexture.dispose(), this.sdfData = new Uint32Array(this.maxSdfs * b * 4), this.sdfFloatData = new Float32Array(this.sdfData.buffer), this.sdfTexture = m(this.sdfData, b, this.maxSdfs), g = !0), I > this.maxEdits && (this.maxEdits = Math.max(I, this.maxEdits * 2), this.editTexture.dispose(), this.editData = new Uint32Array(this.maxEdits * 4), this.editFloatData = new Float32Array(this.editData.buffer), this.editTexture = m(this.editData, 1, this.maxEdits), g = !0), g;
  }
  encodeEdit(A, I, g, B) {
    if (g > 65535 || B > 65535)
      throw new Error("An SDF edit supports at most 65535 shapes");
    const Q = A * 4, i = II(I.rgbaBlendMode) | (I.invert ? 256 : 0);
    let D = this.setEditUint(Q, i);
    return D = this.setEditUint(Q + 1, g | B << 16) || D, D = this.setEditFloat(Q + 2, I.softEdge) || D, D = this.setEditFloat(Q + 3, I.sdfSmooth) || D, D;
  }
  encodeSdf(A, I, g, B, Q, E) {
    const i = A * b * 4, D = gI(I.type) | (I.invert ? 256 : 0);
    let s = this.setSdfFloat(i, g.x);
    return s = this.setSdfFloat(i + 1, g.y) || s, s = this.setSdfFloat(i + 2, g.z) || s, s = this.setSdfUint(i + 3, D) || s, s = this.setSdfFloat(i + 4, B.x) || s, s = this.setSdfFloat(i + 5, B.y) || s, s = this.setSdfFloat(i + 6, B.z) || s, s = this.setSdfFloat(i + 7, B.w) || s, s = this.setSdfFloat(i + 8, Q.x) || s, s = this.setSdfFloat(i + 9, Q.y) || s, s = this.setSdfFloat(i + 10, Q.z) || s, s = this.setSdfUint(i + 11, 0) || s, s = this.setSdfFloat(i + 12, E.x) || s, s = this.setSdfFloat(i + 13, E.y) || s, s = this.setSdfFloat(i + 14, E.z) || s, s = this.setSdfFloat(i + 15, E.w) || s, s = this.setSdfFloat(i + 16, I.color.r) || s, s = this.setSdfFloat(i + 17, I.color.g) || s, s = this.setSdfFloat(i + 18, I.color.b) || s, s = this.setSdfFloat(i + 19, I.opacity) || s, s;
  }
  setSdfUint(A, I) {
  ...
  setSdfFloat(A, I) {
    return eA[0] = I, this.setSdfUint(A, FA[0]);
  }
  setEditUint(A, I) {
  ...
  setEditFloat(A, I) {
    return eA[0] = I, this.setEditUint(A, FA[0]);
  }
};
GA.emptyTexture = m(new Uint32Array(4), 1, 1);
let p = GA;
function II(C) {
  switch (C) {
    case "multiply":
...
function m(C, A, I) {
  ...
  return g.internalFormat = "RGBA32UI", g.magFilter = o.NearestFilter, g.minFilter = o.NearestFilter, g.generateMipmaps = !1, g.needsUpdate = !0, g;
}
function BI() {
  return n.get_raycast_buffer();
}
function QI() {
  return n.get_raycast_buffer2();
}
function CI(C, A, I, g, B, Q, E, i, D, s) {
  return n.raycast_splat_buffers(C, A, I, g, B, Q, E, i, D, s);
}
function EI() {
  return {
    __proto__: null,
    "./gaussian_splat_rs_bg.js": {
      __proto__: null,
      __wbg___wbindgen_debug_string_dd5d2d07ce9e6c57: function(A, I) {
        const g = cA(I), B = nA(g, n.__wbindgen_malloc, n.__wbindgen_realloc), Q = QA;
        O().setInt32(A + 4, Q, !0), O().setInt32(A + 0, B, !0);
      },
      __wbg___wbindgen_throw_81fc77679af83bc6: function(A, I) {
        throw new Error(wA(A, I));
      },
      __wbg_error_a6fa202b58aa1cd3: function(A, I) {
        let g, B;
        try {
          g = A, B = I, console.error(wA(A, I));
        } finally {
          n.__wbindgen_free(g, B, 1);
        }
      },
      __wbg_get_index_983d655608220248: function(A, I) {
        return A[I >>> 0];
      },
      __wbg_length_0c32cb8543c8e4c8: function(A) {
        return A.length;
      },
      __wbg_length_1e701798fdcaa3b4: function(A) {
        return A.length;
      },
      __wbg_length_526c0f6e4ebae15d: function(A) {
        return A.length;
      },
      __wbg_length_fd4646b401926788: function(A) {
        return A.length;
      },
      ...
      __wbg_new_with_length_41a22191b9bdfd66: function(A) {
        return new Uint32Array(A >>> 0);
      },
      __wbg_prototypesetcall_021fd89d67217368: function(A, I, g) {
        Float64Array.prototype.set.call(DI(A, I), g);
      },
      __wbg_prototypesetcall_3e05eb9545565046: function(A, I, g) {
        Uint8Array.prototype.set.call(sI(A, I), g);
      },
      __wbg_prototypesetcall_66c8e1fb820946be: function(A, I, g) {
        Float32Array.prototype.set.call(RA(A, I), g);
      },
      __wbg_prototypesetcall_e42275e601e14eeb: function(A, I, g) {
        Uint32Array.prototype.set.call(oA(A, I), g);
      },
      __wbg_set_448126769bf7c181: function(A, I, g) {
        A.set(oA(I, g));
      },
      __wbg_set_8ee2d34facb8466e: function() {
        return aI(function(A, I, g) {
          return Reflect.set(A, I, g);
        }, arguments);
      },
      __wbg_stack_3b0d974bbf31e44f: function(A, I) {
        const g = I.stack, B = nA(g, n.__wbindgen_malloc, n.__wbindgen_realloc), Q = QA;
        O().setInt32(A + 4, Q, !0), O().setInt32(A + 0, B, !0);
      },
      __wbg_subarray_0f98d3fb634508ad: function(A, I, g) {
        return A.subarray(I >>> 0, g >>> 0);
      },
      __wbg_subarray_4342405c1ffc86d6: function(A, I, g) {
        return A.subarray(I >>> 0, g >>> 0);
      },
      __wbg_subarray_d51e89458b3fdbf6: function(A, I, g) {
        return A.subarray(I >>> 0, g >>> 0);
      },
      ...
      __wbindgen_init_externref_table: function() {
        const A = n.__wbindgen_externrefs, I = A.grow(4);
        A.set(0, void 0), A.set(I + 0, void 0), A.set(I + 1, null), A.set(I + 2, !0), A.set(I + 3, !1);
      }
    }
  };
}
typeof FinalizationRegistry > "u" || new FinalizationRegistry((C) => n.__wbg_chunkdecoder_free(C >>> 0, 1));
function iI(C) {
  const A = n.__externref_table_alloc();
  return n.__wbindgen_externrefs.set(A, C), A;
}
function cA(C) {
  const A = typeof C;
  if (A == "number" || A == "boolean" || C == null)
    return `${C}`;
  if (A == "string")
    return `"${C}"`;
  if (A == "symbol") {
  ...
  const I = /\[object ([^\]]+)\]/.exec(toString.call(C));
  let g;
  if (I && I.length > 1)
    g = I[1];
  else
...
function RA(C, A) {
  return C = C >>> 0, oI().subarray(C / 4, C / 4 + A);
}
function DI(C, A) {
  return C = C >>> 0, wI().subarray(C / 8, C / 8 + A);
}
function oA(C, A) {
  return C = C >>> 0, tI().subarray(C / 4, C / 4 + A);
}
function sI(C, A) {
  return C = C >>> 0, v().subarray(C / 1, C / 1 + A);
}
let u = null;
function O() {
  return (u === null || u.buffer.detached === !0 || u.buffer.detached === void 0 && u.buffer !== n.memory.buffer) && (u = new DataView(n.memory.buffer)), u;
}
let Z = null;
function oI() {
...
let W = null;
function wI() {
  return (W === null || W.byteLength === 0) && (W = new Float64Array(n.memory.buffer)), W;
}
function wA(C, A) {
  return C = C >>> 0, cI(C, A);
}
let T = null;
function tI() {
  return (T === null || T.byteLength === 0) && (T = new Uint32Array(n.memory.buffer)), T;
}
let X = null;
function v() {
...
function aI(C, A) {
  try {
    ...
    const g = iI(I);
    n.__wbindgen_exn_store(g);
  }
}
function nA(C, A, I) {
  if (I === void 0) {
    const i = P.encode(C), D = A(i.length, 1) >>> 0;
    return v().subarray(D, D + i.length).set(i), QA = i.length, D;
  }
  let g = C.length, B = A(g, 1) >>> 0;
  const Q = v();
  let E = 0;
  for (; E < g; E++) {
    const i = C.charCodeAt(E);
    if (i > 127) break;
    Q[B + E] = i;
  }
  if (E !== g) {
    E !== 0 && (C = C.slice(E)), B = I(B, g, g = E + C.length * 3, 1) >>> 0;
    const i = v().subarray(B + E, B + g), D = P.encodeInto(C, i);
    E += D.written, B = I(B, g, E, 1) >>> 0;
  }
  return QA = E, B;
}
let $ = new TextDecoder("utf-8", { ignoreBOM: !0, fatal: !0 });
$.decode();
const eI = 2146435072;
let tA = 0;
function cI(C, A) {
  return tA += A, tA >= eI && ($ = new TextDecoder("utf-8", { ignoreBOM: !0, fatal: !0 }), $.decode(), tA = A), $.decode(v().subarray(C, C + A));
}
const P = new TextEncoder();
"encodeInto" in P || (P.encodeInto = function(C, A) {
  const I = P.encode(C);
  return A.set(I), {
    read: C.length,
    written: I.length
...
let QA = 0, n;
function yI(C, A) {
  return n = C.exports, u = null, Z = null, W = null, T = null, X = null, n.__wbindgen_start(), n;
}
async function hI(C, A) {
  if (typeof Response == "function" && C instanceof Response) {
    ...
    const g = await WebAssembly.instantiate(C, A);
    return g instanceof WebAssembly.Instance ? { instance: g, module: C } : g;
  }
  function I(g) {
    switch (g) {
      case "basic":
...
async function GI(C) {
  ...
  C !== void 0 && (Object.getPrototypeOf(C) === Object.prototype ? { module_or_path: C } = C : console.warn("using deprecated parameters for the initialization function; pass a single object instead"));
  const A = EI();
  (typeof C == "string" || typeof Request == "function" && C instanceof Request || typeof URL == "function" && C instanceof URL) && (C = fetch(C));
  const { instance: I, module: g } = await hI(await C, A);
  return yI(I);
}
const LA = 11, JA = 11, L = 1 << LA, CA = 1 << JA, UA = 1;
var YA = /* @__PURE__ */ ((C) => (C.PLY = "ply", C.SPZ = "spz", C))(YA || {});
const gg = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  SPLAT_TEX_HEIGHT: CA,
  SPLAT_TEX_HEIGHT_BITS: JA,
  SPLAT_TEX_MIN_HEIGHT: UA,
  SPLAT_TEX_WIDTH: L,
  SPLAT_TEX_WIDTH_BITS: LA,
  SplatFileType: YA
}, Symbol.toStringTag, { value: "Module" })), lA = Number.parseInt(o.REVISION), dA = lA >= 179, HA = 1e3, z = new Float32Array(1), sA = new Uint32Array(z.buffer), hA = "Float16Array" in globalThis, j = hA ? new globalThis.Float16Array(1) : null, KA = new Uint16Array(j == null ? void 0 : j.buffer);
function AA(C) {
  return z[0] = C, sA[0];
}
function IA(C) {
...
const Y = hA ? NI : FI, l = hA ? RI : nI;
function NI(C) {
  return j[0] = C, KA[0];
}
function FI(C) {
  z[0] = C;
  const A = sA[0], I = A >> 31 & 1, g = A >> 23 & 255, B = A & 8388607, Q = I << 15;
  if (g === 255)
    return B !== 0 ? Q | 32767 : Q | 31744;
  const E = g - 127 + 15;
  if (E >= 31)
    return Q | 31744;
  if (E <= 0) {
...
function nI(C) {
  const A = C >> 15 & 1, I = C >> 10 & 31, g = C & 1023;
  let B;
  if (I === 0)
    if (g === 0)
      B = A << 31;
      ...
      for (; (Q & 1024) === 0; )
        Q <<= 1, E--;
      Q &= 1023;
      const i = E + 127, D = Q << 13;
      B = A << 31 | i << 23 | D;
    }
  else if (I === 31)
    g === 0 ? B = A << 31 | 2139095040 : B = A << 31 | 2143289344;
  else {
    const Q = I - 15 + 127, E = g << 13;
    B = A << 31 | Q << 23 | E;
  }
  return sA[0] = B, z[0];
}
...
function uA(C, A, I, g, B, Q, E, i, D, s, a, y, c, e, t, h) {
  const w = A * 4, [G, N] = C;
  G[w] = AA(I), G[w + 1] = AA(g), G[w + 2] = AA(B);
  const R = o.MathUtils.clamp(c, 0, HA);
  if (R > 1) {
    const S = 0.25 * (Math.sqrt(1 + Math.E * Math.log(R)) - 1);
    G[w + 3] = Y(1) | Y(S) << 16;
  } else
    G[w + 3] = Y(R);
  N[w] = Y(e) | Y(t) << 16, N[w + 1] = Y(h) | Y(Math.log(Q)) << 16, N[w + 2] = Y(Math.log(E)) | Y(Math.log(i)) << 16, N[w + 3] = xA(D, s, a, y);
}
function yA(C, A) {
  const I = LI, g = A * 4, [B, Q] = C;
  I.center.x = IA(B[g]), I.center.y = IA(B[g + 1]), I.center.z = IA(B[g + 2]);
  const E = B[g + 3], i = E >>> 16;
  if (i === 0)
    I.opacity = l(E & 65535);
  else {
    const D = l(i);
    if (D > 0) {
      ...
      I.opacity = Math.min(
        HA,
        Math.exp((s * s - 1) / Math.E)
      );
    } else
      I.opacity = l(E & 65535);
  }
  return I.color.r = l(Q[g] & 65535), I.color.g = l(Q[g] >>> 16), I.color.b = l(Q[g + 1] & 65535), I.scales.x = Math.exp(l(Q[g + 1] >>> 16)), I.scales.y = Math.exp(l(Q[g + 2] & 65535)), I.scales.z = Math.exp(l(Q[g + 2] >>> 16)), qA(Q[g + 3], I.quaternion), I;
}
const SI = new o.Vector3(), MI = new o.Vector3(), kI = new o.Quaternion(), rI = new o.Color(), LI = {
  center: SI,
  scales: MI,
  quaternion: kI,
  color: rI,
...
function q(C) {
  const A = L, I = Math.max(
    UA,
    Math.min(CA, Math.ceil(C / A))
  ), g = Math.ceil(C / (A * I)), B = A * I * g;
...
`;
function xA(C, A, I, g) {
  const B = Math.sqrt(C * C + A * A + I * I + g * g), Q = (g < 0 ? -C : C) / B, E = (g < 0 ? -A : A) / B, i = (g < 0 ? -I : I) / B, D = (g < 0 ? -g : g) / B, s = 2 * Math.acos(D), a = Math.sqrt(Q * Q + E * E + i * i), y = a < 1e-6 ? 1 : Q / a, c = a < 1e-6 ? 0 : E / a, e = a < 1e-6 ? 0 : i / a, t = Math.abs(y) + Math.abs(c) + Math.abs(e);
  let h = y / t, w = c / t;
  if (e < 0) {
    const k = h;
    h = (1 - Math.abs(w)) * (h >= 0 ? 1 : -1), w = (1 - Math.abs(k)) * (w >= 0 ? 1 : -1);
  }
  const G = h * 0.5 + 0.5, N = w * 0.5 + 0.5, R = Math.round(G * 1023), S = Math.round(N * 1023);
  return Math.round(s * (4095 / Math.PI)) << 20 | S << 10 | R;
}
function qA(C, A) {
  const I = C & 1023, g = C >>> 10 & 1023, B = C >>> 20 & 4095, Q = I / 1023, E = g / 1023;
  let i = (Q - 0.5) * 2, D = (E - 0.5) * 2;
  const s = 1 - (Math.abs(i) + Math.abs(D)), a = Math.max(-s, 0);
  i += i >= 0 ? -a : a, D += D >= 0 ? -a : a;
  const y = Math.sqrt(i * i + D * D + s * s), c = y < 1e-6 ? 0 : i / y, e = y < 1e-6 ? 0 : D / y, t = y < 1e-6 ? 0 : s / y, w = B / 4095 * Math.PI * 0.5, G = Math.sin(w), N = Math.cos(w);
  return A.set(c * G, e * G, t * G, N), A;
}
function bA(C, A, I, g, B) {
  const Q = C.getContext(), E = C.properties.get(A), i = E == null ? void 0 : E.__webglTexture;
  if (!i)
    throw new Error("texture not found");
  const D = Q.getParameter(Q.UNPACK_FLIP_Y_WEBGL), s = Q.getParameter(Q.UNPACK_PREMULTIPLY_ALPHA_WEBGL);
  C.state.activeTexture(Q.TEXTURE0), C.state.bindTexture(Q.TEXTURE_2D, i), Q.bindBuffer(Q.PIXEL_UNPACK_BUFFER, null), Q.pixelStorei(Q.UNPACK_FLIP_Y_WEBGL, !1), Q.pixelStorei(Q.UNPACK_PREMULTIPLY_ALPHA_WEBGL, !1), Q.texSubImage2D(
    Q.TEXTURE_2D,
    0,
    0,
    0,
    ...
    Q.UNSIGNED_INT,
    B
  ), C.state.unbindTexture(), Q.pixelStorei(Q.UNPACK_FLIP_Y_WEBGL, D), Q.pixelStorei(Q.UNPACK_PREMULTIPLY_ALPHA_WEBGL, s);
}
function mA(C) {
  return {
    timer: C ?? new o.Timer(),
...
const Bg = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  IDENT_VERTEX_SHADER: pA,
  decodeQuatOctXy1010R12: qA,
  decodeSplat: yA,
  encodeQuatOctXy1010R12: xA,
  encodeSplat: uA,
  floatBitsToUint: AA,
  fromHalf: l,
  getTextureSize: q,
...
function KI(C) {
  ...
  try {
    ...
    return new Worker(
      "data:text/javascript;charset=utf-8," + encodeURIComponent(TA),
      {
        name: C == null ? void 0 : C.name
      }
    );
...
const DA = class DA {
  constructor() {
    this.messages = {}, this.worker = new KI(), this.worker.onmessage = (A) => this.onMessage(A), ZA.then((A) => {
      this.worker.postMessage({ name: "init-wasm", module: A });
    });
  }
  onMessage(A) {
    const { id: I, result: g, error: B, status: Q } = A.data, E = this.messages[I];
    if (E) {
      if (Q !== void 0) {
        E.statusQueue = E.statusQueue.then(() => {
          var i;
          if (this.messages[I] === E)
            return (i = E.onStatus) == null ? void 0 : i.call(E, Q);
        }), E.statusQueue.catch(() => {
        });
        return;
      }
  ...
  async call(A, I, g = {}) {
    const B = ++DA.currentId, Q = new Promise((E, i) => {
      this.messages[B] = {
        resolve: (D) => E(D),
        reject: i,
        onStatus: g.onStatus,
...
class uI {
  constructor(A = 4) {
    this.numWorkers = 0, this.freelist = [], this.idleWorkerTimeouts = /* @__PURE__ */ new Map(), this.queue = [], this.maxWorkers = A;
  }
  async withWorker(A) {
  ...
  async allocWorker() {
    ...
    if (A) {
      const I = this.idleWorkerTimeouts.get(A);
      return I !== void 0 && (clearTimeout(I), this.idleWorkerTimeouts.delete(A)), A;
    }
  ...
  freeWorker(A) {
    ...
    const g = setTimeout(() => {
      this.idleWorkerTimeouts.delete(A);
      const B = this.freelist.indexOf(A);
      B !== -1 && (this.freelist.splice(B, 1), A.dispose(), this.numWorkers -= 1);
    }, fI);
    this.idleWorkerTimeouts.set(A, g);
  }
}
const pI = new uI();
class xI extends PA {
  load(A, I, g, B) {
    return this.loadInternal({ url: A, onLoad: I, onProgress: g, onError: B });
  }
  async loadAsync(A, I) {
  ...
  loadInternal({
    ...
    const y = g instanceof ArrayBuffer ? new Uint8Array(g) : g, c = y ? void 0 : this.manager.resolveURL((this.path ?? "") + (I ?? ""));
    let e = E == null ? void 0 : E.getReader();
    this.manager.itemStart(c ?? ""), pI.withWorker(async (t) => {
      const h = async (R) => {
        const { loaded: S, total: J } = R;
        if (S !== void 0 && s)
          try {
            s(
            ...
            console.error("Progress callback failed", k);
          }
        if (R.nextChunk) {
          let k;
          if (!e)
            ...
            const { done: x, value: VA } = await e.read().catch(async (vA) => {
              throw await t.call("nextChunk", { chunk: new Uint8Array(0) }), vA;
            });
            x ? (e.releaseLock(), e = void 0, k = new Uint8Array(0)) : k = VA;
          }
          await t.call("nextChunk", { chunk: k });
        }
      }, w = c ? new URL(c, window.location.href).toString() : void 0, G = await t.call(
        "loadSplats",
        {
          url: w,
          requestHeader: this.requestHeader,
          withCredentials: this.withCredentials,
          fileBytes: y == null ? void 0 : y.slice(),
          fileType: B,
          pathName: c || Q,
          chunked: E !== void 0,
          chunkedLength: i
      ...
      if (e) {
        ...
        e.releaseLock(), e = void 0;
      }
      this.manager.itemError(c ?? ""), a == null || a(t);
    }).finally(() => {
...
const r = class r {
  constructor(A = {}) {
    this.maxSplats = 0, this.numSplats = 0, this.splatArrays = [
      new Uint32Array(0),
      new Uint32Array(0)
    ], this.extra = {}, this.isInitialized = !1, this.needsUpdate = !0, this.shTextures = {}, this.textures = [r.emptyTexture, r.emptyTexture], this.initialized = Promise.resolve(this), this.reinitialize(A);
  }
  reinitialize(A) {
    this.isInitialized = !1, this.disposeTextures(), this.extra = {}, this.maxSplats = A.maxSplats ?? 0, this.needsUpdate = !0, A.url || A.fileBytes || A.stream || A.construct ? this.initialized = this.asyncInitialize(A).then(() => (this.isInitialized = !0, this)) : (this.initialize(A), this.isInitialized = !0, this.initialized = Promise.resolve(this));
  }
  initialize(A) {
    this.disposeTextures(), this.extra = A.extra ?? {}, A.splatArrays ? (this.splatArrays = A.splatArrays, this.maxSplats = Math.floor(
      Math.min(this.splatArrays[0].length, this.splatArrays[1].length) / 4
    ), this.maxSplats = Math.floor(this.maxSplats / L) * L, this.numSplats = Math.min(
      this.maxSplats,
      A.numSplats ?? this.maxSplats
    )) : (this.maxSplats = A.maxSplats ?? 0, this.numSplats = 0, this.splatArrays = [new Uint32Array(0), new Uint32Array(0)]), this.needsUpdate = !0;
  }
  async asyncInitialize(A) {
    var B;
    const I = new xI();
    (A.fileBytes || A.url || A.stream) && await I.loadInternalAsync({
      splats: this,
      url: A.url,
    ...
    const g = (B = A.construct) == null ? void 0 : B.call(A, this);
    g instanceof Promise && await g;
  }
  dispose() {
    this.disposeTextures(), this.splatArrays = [new Uint32Array(0), new Uint32Array(0)], this.extra = {};
  }
  disposeTextures() {
    for (const A of this.textures ?? [])
      A !== r.emptyTexture && A.dispose();
    this.textures = [r.emptyTexture, r.emptyTexture];
    for (const A of Object.values(this.shTextures ?? {}))
      A == null || A.dispose();
    this.shTextures = {};
  }
  getNumSplats() {
  ...
  ensureSplats(A) {
    const I = this.splatArrays[0].length / 4, g = A <= this.maxSplats ? this.maxSplats : Math.max(A, 2 * this.maxSplats);
    if (g > I) {
      this.maxSplats = q(Math.max(1, g)).maxSplats;
      const B = new Uint32Array(this.maxSplats * 4), Q = new Uint32Array(this.maxSplats * 4);
      B.set(this.splatArrays[0]), Q.set(this.splatArrays[1]), this.splatArrays = [B, Q], this.needsUpdate = !0;
    }
    return this.splatArrays;
  }
  getSplat(A) {
    if (A < 0 || A >= this.numSplats)
      throw new Error("Invalid splat index");
    return yA(this.splatArrays, A);
  }
  setSplat(A, I, g, B, Q, E) {
    const i = this.ensureSplats(A + 1);
    uA(
      i,
      A,
      I.x,
      ...
      E.g,
      E.b
    ), this.numSplats = Math.max(this.numSplats, A + 1), this.needsUpdate = !0;
  }
  pushSplat(A, I, g, B, Q) {
    this.setSplat(this.numSplats, A, I, g, B, Q);
  }
  forEachCenter(A) {
    const [I, g] = this.splatArrays, B = new Float32Array(
      I.buffer,
      I.byteOffset,
      I.length
    );
    for (let Q = 0; Q < this.numSplats; Q += 1) {
      const E = Q * 4, i = g[E + 1] >>> 16, D = g[E + 2] & 65535, s = g[E + 2] >>> 16;
      if (i === 64512 && D === 64512 && s === 64512) {
        A(Q, Number.NaN, Number.NaN, Number.NaN);
        continue;
      }
      A(Q, B[E], B[E + 1], B[E + 2]);
    }
  ...
  forEachSplat(A) {
    for (let I = 0; I < this.numSplats; I += 1) {
      const g = yA(this.splatArrays, I);
      A(
        I,
        g.center,
        g.scales,
  ...
  getSplatTextures() {
    if (this.maxSplats === 0 || this.splatArrays[0].length === 0)
      return [r.emptyTexture, r.emptyTexture];
    const { width: A, height: I, depth: g } = q(this.maxSplats), B = this.textures[0].image.data;
    return this.textures[0] === r.emptyTexture || this.textures[0].image.width !== A || this.textures[0].image.height !== I || this.textures[0].image.depth !== g || B === null || B.buffer !== this.splatArrays[0].buffer ? (this.disposeMainTextures(), this.textures = [
      _(this.splatArrays[0], A, I, g),
      _(this.splatArrays[1], A, I, g)
    ]) : this.needsUpdate && (this.textures[0].needsUpdate = !0, this.textures[1].needsUpdate = !0), this.textures;
  }
  disposeMainTextures() {
    for (const A of this.textures)
      A !== r.emptyTexture && A.dispose();
  ...
  getShTextures() {
    return this.shTextures.sh1 = this.ensureShTexture("sh1", this.shTextures.sh1), this.shTextures.sh2 = this.ensureShTexture("sh2", this.shTextures.sh2), this.shTextures.sh3a = this.ensureShTexture("sh3a", this.shTextures.sh3a), this.shTextures.sh3b = this.ensureShTexture("sh3b", this.shTextures.sh3b), this.shTextures;
  }
  ensureShTexture(A, I) {
    let g = I;
    const B = this.extra[A];
    if (!B) {
      g == null || g.dispose();
      return;
    }
    const { width: Q, height: E, depth: i, maxSplats: D } = q(
      Math.max(1, B.length / 4)
    );
    let s = B;
    return B.length < D * 4 && (s = new Uint32Array(D * 4), s.set(B), this.extra[A] = s), g && (g.image.width !== Q || g.image.height !== E || g.image.depth !== i || g.image.data === null || g.image.data.buffer !== s.buffer) && (g == null || g.dispose(), g = void 0), g ? this.needsUpdate && (g.needsUpdate = !0) : g = _(s, Q, E, i), g;
  }
};
r.emptyTexture = _(null, 1, 1, 1);
let H = r;
function _(C, A, I, g) {
  const B = new o.DataArrayTexture(
    C,
    A,
    I,
    g
  );
  return B.format = o.RGBAIntegerFormat, B.type = o.UnsignedIntType, B.internalFormat = "RGBA32UI", B.magFilter = o.NearestFilter, B.minFilter = o.NearestFilter, B.generateMipmaps = !1, B.needsUpdate = !0, B;
}
const qI = new o.Matrix4(), bI = new o.Matrix3(), mI = new o.Vector3(), ZI = new o.Vector3();
class gA extends o.Object3D {
  constructor(A = {}) {
    var B;
    super(), this.isInitialized = !1, this.numSplats = 0, this.recolor = new o.Color(1, 1, 1), this.opacity = 1, this.maxSh = 3, this.edits = null, this.sdfEdits = null, this.version = 0, this.sortVersion = 0, this.mappingVersion = 0, this.lastNumSplats = -1, this.lastMaxSh = -1, this.lastMatrixWorld = new o.Matrix4(), this.hasLastMatrixWorld = !1, this.lastRecolor = new o.Vector4().setScalar(Number.NaN), this.viewOrigin = new o.Vector3(), this.lastViewOrigin = new o.Vector3().setScalar(Number.NaN), this.sdfCoordinateOrigin = new o.Vector3(), this.splats = A.splats ?? new H({ maxSplats: A.maxSplats }), this.numSplats = this.splats.getNumSplats(), this.editable = A.editable ?? !0, this.raycastable = A.raycastable ?? !0, this.minRaycastOpacity = A.minRaycastOpacity ?? 0.05, this.onFrame = A.onFrame;
    const I = this.splats instanceof H ? this.splats : void 0;
    if (!!(I && (A.url || A.fileBytes || A.stream || A.constructSplats || !I.isInitialized)))
      this.initialized = this.asyncInitialize(A).then(async () => {
        var Q;
        return this.isInitialized = !0, await ((Q = A.onLoad) == null ? void 0 : Q.call(A, this)), this;
      });
    else {
      this.isInitialized = !0;
      const Q = (B = A.onLoad) == null ? void 0 : B.call(A, this);
      this.initialized = Q instanceof Promise ? Q.then(() => this) : Promise.resolve(this);
    }
  }
  async asyncInitialize(A) {
    var g;
    const I = this.splats;
    I instanceof H && ((A.url || A.fileBytes || A.stream || A.constructSplats) && I.reinitialize({
      url: A.url,
      fileBytes: A.fileBytes,
      fileType: A.fileType,
      fileName: A.fileName,
      stream: A.stream,
      streamLength: A.streamLength,
      maxSplats: A.maxSplats,
      construct: A.constructSplats,
      onProgress: A.onProgress
    }), await I.initialized), this.numSplats = ((g = this.splats) == null ? void 0 : g.getNumSplats()) ?? 0, this.updateMappingVersion();
  }
  pushSplat(A, I, g, B, Q) {
    var E;
    this.splats instanceof H && this.splats.pushSplat(A, I, g, B, Q), this.numSplats = ((E = this.splats) == null ? void 0 : E.getNumSplats()) ?? this.numSplats;
  }
  forEachSplat(A) {
    var I;
  ...
  dispose() {
    var A, I;
    (A = this.sdfEdits) == null || A.dispose(), this.sdfEdits = null, (I = this.splats) == null || I.dispose(), this.splats = void 0;
  }
  getBoundingBox(A = !0) {
    var E, i;
    ...
    const I = new o.Vector3().setScalar(Number.POSITIVE_INFINITY), g = new o.Vector3().setScalar(Number.NEGATIVE_INFINITY), B = new o.Vector3();
    if (A)
      return (E = this.splats) == null || E.forEachCenter((D, s, a, y) => {
        Number.isNaN(s) || (B.set(s, a, y), I.min(B), g.max(B));
      }), new o.Box3(I, g);
    const Q = [-1, 1];
    return (i = this.splats) == null || i.forEachSplat((D, s, a, y) => {
      for (const c of Q)
        for (const e of Q)
          for (const t of Q)
            B.set(c * a.x, e * a.y, t * a.z).applyQuaternion(y).add(s), I.min(B), g.max(B);
    }), new o.Box3(I, g);
  }
  frameUpdate({ time: A, deltaTime: I, camera: g, globalEdits: B }) {
    var t, h;
    (t = this.onFrame) == null || t.call(this, { mesh: this, time: A, deltaTime: I });
    const Q = this.splats;
    if (!Q)
      return;
    this.splats = Q;
    let E = !1, i = !1;
    const D = Q.getNumSplats();
    Q !== this.lastSource && (this.lastSource = Q, E = !0, i = !0), D !== this.lastNumSplats && (this.lastNumSplats = D, this.numSplats = D, this.mappingVersion += 1, E = !0, i = !0), Q.needsUpdate && (E = !0, i = !0), this.maxSh !== this.lastMaxSh && (this.lastMaxSh = this.maxSh, E = !0), this.maxSh > 0 && Q.getNumSh() > 0 && (g.getWorldPosition(this.viewOrigin), this.viewOrigin.equals(this.lastViewOrigin) || (this.lastViewOrigin.copy(this.viewOrigin), E = !0)), this.updateWorldMatrix(!0, !1), (!this.hasLastMatrixWorld || !this.lastMatrixWorld.equals(this.matrixWorld)) && (this.lastMatrixWorld.copy(this.matrixWorld), this.hasLastMatrixWorld = !0, E = !0, i = !0);
    const s = new o.Vector4(
      this.recolor.r,
      this.recolor.g,
      this.recolor.b,
      this.opacity
    );
    s.equals(this.lastRecolor) || (this.lastRecolor.copy(s), E = !0);
    const a = /* @__PURE__ */ new Set();
    if (this.editable) {
      for (const w of B) a.add(w);
      if (this.edits)
        for (const w of this.edits) a.add(w);
      else
        this.traverseVisible((w) => {
          w instanceof BA && a.add(w);
        });
    }
    const c = Array.from(a).sort(
      (w, G) => w.ordering - G.ordering
    ).map((w) => {
      if (w.sdfs) return { edit: w, sdfs: w.sdfs };
      const G = [];
      return w.traverseVisible((N) => {
        N instanceof AI && G.push(N);
      }), { edit: w, sdfs: G };
    });
    c.length > 0 && !this.sdfEdits && (this.sdfEdits = new p({
      maxEdits: c.length,
      maxSdfs: c.reduce((w, G) => w + G.sdfs.length, 0)
    }), E = !0);
    const e = this.sdfCoordinateOrigin.setFromMatrixPosition(
      this.matrixWorld
    );
    (h = this.sdfEdits) != null && h.update(c, e).updated && (E = !0), E && this.updateVersion({ sort: i });
  }
  ...
  raycast(A, I) {
    if (!HI() || !this.raycastable || !(this.splats instanceof H))
      return;
    const { near: g, far: B, ray: Q } = A;
    if (this.numSplats === 0 || !Number.isFinite(this.minRaycastOpacity) || this.minRaycastOpacity >= 1 || g > B)
      return;
    const E = qI.copy(this.matrixWorld).invert(), i = mI.copy(Q.origin).applyMatrix4(E), D = ZI.copy(Q.direction).applyMatrix3(bI.setFromMatrix4(E)), s = BI(), a = QI(), y = s.length / 4, [c, e] = this.splats.splatArrays;
    for (let t = 0; t < this.numSplats; t += y) {
      const h = Math.min(y, this.numSplats - t);
      s.set(c.subarray(t * 4, (t + h) * 4)), a.set(e.subarray(t * 4, (t + h) * 4));
      const w = CI(
        i.x,
        i.y,
        i.z,
...
var WI = `const uint SPLAT_TEX_WIDTH_BITS = 11u;
...
let aA = null;
function XA() {
  return aA || (o.ShaderChunk.splatDefines = WI, aA = {
    splatVertex: XI,
    splatFragment: TI
  }), aA;
}
...
const MA = new o.Matrix4(), d = new o.Vector3(), K = new o.Vector3(), f = new o.Vector3(), kA = new o.Vector3();
function vI(C, A, I) {
  const g = C.elements, B = Math.hypot(g[0], g[1], g[2]), Q = Math.hypot(g[4], g[5], g[6]), E = Math.hypot(g[8], g[9], g[10]);
  A.set(B, Q, E), d.set(g[0], g[1], g[2]), K.set(g[4], g[5], g[6]), f.set(g[8], g[9], g[10]), B > 0 && d.multiplyScalar(1 / B), Q > 0 && K.multiplyScalar(1 / Q), E > 0 && f.multiplyScalar(1 / E);
  const i = +(B > 0) + +(Q > 0) + +(E > 0);
  if (i === 0) {
    I.identity();
    return;
  ...
  B === 0 && d.copy(K).cross(f).normalize(), Q === 0 && K.copy(f).cross(d).normalize(), E === 0 && f.copy(d).cross(K).normalize(), C.determinant() < 0 && d.negate(), MA.makeBasis(d, K, f), I.setFromRotationMatrix(MA).normalize();
}
const M = class M {
  constructor() {
    if (this.time = 0, this.deltaTime = 0, this.viewOrigin = new o.Vector3(), this.viewDirection = new o.Vector3(), this.maxSplats = 0, this.numSplats = 0, this.target = null, this.mapping = [], this.version = -1, this.mappingVersion = -1, this.transformScale = new o.Vector3(), this.transformQuaternion = new o.Quaternion(), !dA)
      throw new Error("Gaussian Splat Lite requires THREE.js r179 or above");
  }
  dispose() {
    var A;
    (A = this.target) == null || A.dispose(), this.target = null;
  }
  getTextures() {
    var A;
    return ((A = this.target) == null ? void 0 : A.textures) ?? M.emptyTextures;
  }
  generateMapping(A) {
    let I = 0;
    const g = A.map((B) => {
      const Q = I;
      return I += Math.ceil(B / L) * L, { base: Q, count: B };
    });
    return { maxSplats: I, mapping: g };
  }
  ensureGenerate({ maxSplats: A }) {
    if (this.target && Math.max(1, A) <= this.maxSplats)
      return !1;
    this.dispose();
    const I = q(Math.max(1, A)), { width: g, height: B, depth: Q } = I;
    this.maxSplats = I.maxSplats, this.target = new o.WebGLArrayRenderTarget(g, B, Q, {
      depthBuffer: !1,
      stencilBuffer: !1,
      generateMipmaps: !1,
      magFilter: o.NearestFilter,
      minFilter: o.NearestFilter,
      format: o.RGBAIntegerFormat,
      type: o.UnsignedIntType
    }), this.target.scissorTest = !0;
    const E = this.target.texture.clone();
    return this.target.textures = [this.target.texture, E], !0;
  }
  getMaterial() {
    let A = M.material;
    return A || (XA(), A = new o.RawShaderMaterial({
      glslVersion: o.GLSL3,
      vertexShader: pA,
      fragmentShader: VI,
      uniforms: PI(),
  ...
  prepareMaterial(A) {
    ...
    const g = this.getMaterial(), B = g.uniforms, [Q, E] = I.getSplatTextures(), i = I.getShTextures();
    I.needsUpdate = !1, B.sourceSplats.value = Q, B.sourceSplats2.value = E, B.numSh.value = Math.min(A.maxSh, I.getNumSh()), B.sh1Texture.value = i.sh1 ?? M.emptyTexture, B.sh2Texture.value = i.sh2 ?? M.emptyTexture, B.sh3TextureA.value = i.sh3a ?? M.emptyTexture, B.sh3TextureB.value = i.sh3b ?? M.emptyTexture, vI(
      A.matrixWorld,
      this.transformScale,
      this.transformQuaternion
    ), B.objectBasis.value.setFromMatrix4(A.matrixWorld), B.objectOffset.value.setFromMatrixPosition(A.matrixWorld), B.objectOffset.value.sub(this.viewOrigin), B.objectLnScale.value.set(
      Math.log(this.transformScale.x),
      Math.log(this.transformScale.y),
      Math.log(this.transformScale.z)
    ), B.objectQuaternion.value.copy(this.transformQuaternion), B.recolor.value.set(
      A.recolor.r,
      A.recolor.g,
      A.recolor.b,
      A.opacity
    ...
    const D = A.sdfEdits;
    return B.numSdfs.value = (D == null ? void 0 : D.numSdfs) ?? 0, B.numEdits.value = (D == null ? void 0 : D.numEdits) ?? 0, B.sdfTexture.value = (D == null ? void 0 : D.sdfTexture) ?? p.emptyTexture, B.editTexture.value = (D == null ? void 0 : D.editTexture) ?? p.emptyTexture, M.fullScreenQuad.material = g, g;
  }
  generate({
    mesh: A,
    base: I,
    ...
    const E = this.prepareMaterial(A).uniforms, i = this.saveRenderState(B), D = Math.ceil((I + g) / L) * L, s = L * CA;
    for (E.targetBase.value = I, E.targetCount.value = g; I < D; ) {
      const a = Math.floor(I / s);
      E.targetLayer.value = a;
      const y = a * s, c = Math.floor((I - y) / L), e = Math.min(
        CA,
        Math.ceil((D - y) / L)
      );
      this.target.scissor.set(0, c, L, e - c), B.setRenderTarget(this.target, a), B.xr.enabled = !1, B.autoClear = !1, M.fullScreenQuad.render(B), I += L * (e - c);
    }
    this.resetRenderState(B, i);
  }
  prepareGenerate({
    renderer: A,
    scene: I,
    ...
    B.getWorldPosition(this.viewOrigin), B.getWorldDirection(this.viewDirection), this.time = g.getElapsed(), this.deltaTime = g.getDelta();
    const E = [];
    I.traverse((t) => {
      t instanceof gA && B.layers.test(t.layers) && E.push(t);
    });
    const i = /* @__PURE__ */ new Set();
    I.traverseVisible((t) => {
      if (!(t instanceof BA)) return;
      let h = t.parent;
      for (; h && !(h instanceof gA); )
        h = h.parent;
      h || i.add(t);
    });
    for (const t of E)
      try {
        ...
        console.error("SplatMesh frame update failed", h);
      }
    const D = [];
    I.traverseVisible((t) => {
      t instanceof gA && B.layers.test(t.layers) && D.push(t);
    });
    const { maxSplats: s, mapping: a } = this.generateMapping(
      D.map((t) => t.numSplats)
    );
    this.mapping = [], this.numSplats = 0, a.forEach(({ base: t, count: h }, w) => {
      const G = D[w];
      !G.splats || h <= 0 || (this.mapping.push({
        node: G,
        version: G.version,
        sortVersion: G.sortVersion,
        mappingVersion: G.mappingVersion,
        base: t,
        count: h
      }), this.numSplats = Math.max(this.numSplats, t + h));
    });
    const { splatsUpdated: y, mappingUpdated: c, sortUpdated: e } = Q.checkVersions(this.mapping);
    return this.version = Q.version + (y ? 1 : 0), this.mappingVersion = Q.mappingVersion + (c ? 1 : 0), {
      version: this.version,
      sortUpdated: e,
      generate: () => {
        this.ensureGenerate({ maxSplats: s });
        for (const { node: t, base: h, count: w } of this.mapping)
          this.generate({ mesh: t, base: h, count: w, renderer: A });
      }
    };
  }
  checkVersions(A) {
    return this.mapping.length !== A.length ? { splatsUpdated: !0, mappingUpdated: !0, sortUpdated: !0 } : this.mapping.some((g, B) => {
      const Q = A[B];
      return g.node !== Q.node || g.base !== Q.base || g.count !== Q.count || g.mappingVersion !== Q.mappingVersion;
    }) ? { splatsUpdated: !0, mappingUpdated: !0, sortUpdated: !0 } : {
      splatsUpdated: this.mapping.some(
        (g, B) => g.version !== A[B].version
      ),
      mappingUpdated: !1,
      sortUpdated: this.mapping.some(
        (g, B) => g.sortVersion !== A[B].sortVersion
      )
    };
  }
  saveRenderState(A) {
...
M.emptyTexture = (() => {
  const { width: A, height: I, depth: g, maxSplats: B } = q(1), Q = new o.DataArrayTexture(
    new Uint32Array(B * 4),
    A,
    I,
    g
  );
  return Q.format = o.RGBAIntegerFormat, Q.type = o.UnsignedIntType, Q.internalFormat = "RGBA32UI", Q.needsUpdate = !0, Q;
})(), M.emptyTextures = [
  M.emptyTexture,
  M.emptyTexture
], M.material = null, M.fullScreenQuad = new jA(
  new o.RawShaderMaterial({ visible: !1 })
);
let U = M;
function PI() {
  return {
    ...
    targetBase: { value: 0 },
    targetCount: { value: 0 },
    sourceSplats: { value: U.emptyTexture },
    sourceSplats2: { value: U.emptyTexture },
    numSh: { value: 0 },
    sh1Texture: { value: U.emptyTexture },
    sh2Texture: { value: U.emptyTexture },
    sh3TextureA: { value: U.emptyTexture },
    sh3TextureB: { value: U.emptyTexture },
    objectBasis: { value: new o.Matrix3() },
    objectOffset: { value: new o.Vector3() },
    objectLnScale: { value: new o.Vector3() },
    objectQuaternion: { value: new o.Quaternion() },
    recolor: { value: new o.Vector4(1, 1, 1, 1) },
    numSdfs: { value: 0 },
    numEdits: { value: 0 },
    sdfTexture: { value: p.emptyTexture },
    editTexture: { value: p.emptyTexture }
  };
}
class jI extends o.InstancedBufferGeometry {
  constructor() {
    super(), this.setAttribute("position", new o.BufferAttribute(zI, 3)), this.setIndex(new o.BufferAttribute(OI, 1));
  }
}
const zI = new Float32Array([
  -1,
  -1,
  0,
  1,
  ...
  1,
  0
]), OI = new Uint16Array([0, 1, 2, 0, 2, 3]), V = new o.Vector3();
function _I(C) {
  const A = C.getWorldScale(V);
  return (A.x + A.y + A.z) / 3;
}
const F = class F extends o.Mesh {
  constructor(A) {
    if (!A)
    ...
    const I = F.makeUniforms();
    Object.assign(I, A.extraUniforms ?? {});
    const g = XA(), B = A.premultipliedAlpha ?? !0, Q = new jI(), E = new o.ShaderMaterial({
      glslVersion: o.GLSL3,
    ...
    super(Q, E), this.renderSize = new o.Vector2(), this.lastFrame = -1, this.updateTimeoutId = -1, this.orderingTexture = null, this.maxSplats = 0, this.activeSplats = 0, this.accumulators = [], this.sorting = !1, this.sortDirty = !1, this.lastSortTime = 0, this.sortWorker = null, this.sortedCenter = new o.Vector3().setScalar(Number.NEGATIVE_INFINITY), this.sortedDir = new o.Vector3().setScalar(0), this.sortCenterCache = new OA(), this.sortCentersRevision = 0, this.uploadedSortCentersRevision = -1, this.updateRunning = !1, this.updatePromise = Promise.resolve(), this.queuedUpdate = null, this.disposed = !1, this.superXY = 1, this.material = E, this.uniforms = I, this.frustumCulled = !1, this.renderer = A.renderer, this.onDirty = A.onDirty, this.dirty = !0, this.autoUpdate = A.autoUpdate ?? !0, this.preUpdate = A.preUpdate ?? !0, this.maxStdDev = A.maxStdDev ?? Math.sqrt(8), this.minPixelRadius = A.minPixelRadius ?? 1, this.maxPixelRadius = A.maxPixelRadius ?? 512, this.minAlpha = A.minAlpha ?? 0.5 * (1 / 255), this.enable2DGS = A.enable2DGS ?? !1, this.preBlurAmount = A.preBlurAmount ?? 0, this.blurAmount = A.blurAmount ?? 0.3, this.clipXY = A.clipXY ?? 1.25, this.focalAdjustment = A.focalAdjustment ?? 2, this.sortRadial = A.sortRadial ?? !1, this.minSortIntervalMs = A.minSortIntervalMs ?? 0;
    const { timer: i, ownsTimer: D } = mA(A.timer);
    this.timer = i, this.ownsTimer = D, this.display = this.createAccumulator(), this.current = this.display, this.accumulators.push(this.createAccumulator());
    const s = this.renderer.getContext().getExtension("WEBGL_provoking_vertex");
    if (s && s.provokingVertexWEBGL(
      s.FIRST_VERTEX_CONVENTION_WEBGL
    ), A.target) {
      const {
        width: a,
        height: y,
        ...
        superXY: e,
        ...t
      } = A.target, h = Math.max(1, Math.min(4, e ?? 1));
      if (a * h > 8192 || y * h > 8192)
        throw new Error("Target size too large");
      this.superXY = h;
      const w = a * h, G = y * h, N = {
        format: o.RGBAFormat,
        type: o.UnsignedByteType,
        colorSpace: o.SRGBColorSpace,
        ...t
      };
      this.target = new o.WebGLRenderTarget(
        w,
        G,
        N
      ), c && (this.backTarget = new o.WebGLRenderTarget(
        w,
        G,
        N
      ));
  ...
  static makeUniforms() {
    return {
      ...
      renderSize: { value: new o.Vector2() },
      // Near and far plane distances
      near: { value: 0.1 },
      far: { value: 1e3 },
      // SplatAccumulator to view transformation quaternion
      renderToViewQuat: { value: new o.Quaternion() },
      // SplatAccumulator to view transformation translation
      renderToViewPos: { value: new o.Vector3() },
      ...
      ordering: { type: "t", value: F.emptyOrdering },
      // Gsplat collection to render
      splats: { type: "t", value: U.emptyTexture },
      splats2: { type: "t", value: U.emptyTexture },
  ...
  dispose() {
    ...
    this.disposed = !0, this.queuedUpdate = null, this.target && (this.target.dispose(), this.target = void 0), this.backTarget && (this.backTarget.dispose(), this.backTarget = void 0), this.orderingTexture && (this.orderingTexture.dispose(), this.orderingTexture = null);
    const A = /* @__PURE__ */ new Set();
    ...
    for (const I of A)
      I.dispose();
    this.sortWorker && (this.sortWorker.dispose(), this.sortWorker = null), this.sortCenterCache.dispose(), this.geometry.dispose(), this.material.dispose();
  }
  setDirty() {
    var A;
  ...
  releaseAccumulator(A) {
    this.accumulators.push(A);
  }
  onBeforeRender(A, I, g) {
    var N;
    const B = F.gaussianSplatOverride ?? this, Q = A.info.render.frame, E = Q !== B.lastFrame;
    B.lastFrame = Q;
    const i = A.getRenderTarget(), D = $I(i);
    if (i) {
      if (B.renderSize.set(
        i.width,
        i.height
      ), D && B.renderSize.x === 1 && B.renderSize.y === 1) {
        const R = (N = A.xr.getSession()) == null ? void 0 : N.renderState.baseLayer;
        R && (B.renderSize.x = R.framebufferWidth, B.renderSize.y = R.framebufferHeight);
      }
    } else
      A.getDrawingBufferSize(B.renderSize);
    if (this.uniforms.renderSize.value.copy(B.renderSize), B.autoUpdate && E) {
      const R = B.preUpdate && !A.xr.isPresenting;
      let S = g;
      if (A.xr.isPresenting) {
        const J = A.xr.getCamera();
        S = J.cameras[0] ?? J;
      }
      R ? B.updateInternal({
        scene: I,
    ...
    const s = g;
    this.uniforms.near.value = s.near, this.uniforms.far.value = s.far;
    const a = this.geometry;
    a.instanceCount = B.activeSplats;
    const y = B.display, c = new o.Matrix4().makeTranslation(
      y.viewOrigin
    );
    g.matrixWorld.clone().invert().multiply(c).decompose(
      this.uniforms.renderToViewPos.value,
      this.uniforms.renderToViewQuat.value,
      V
    ), this.uniforms.renderToViewScale.value = (V.x + V.y + V.z) / 3, this.uniforms.maxStdDev.value = B.maxStdDev, this.uniforms.minPixelRadius.value = B.minPixelRadius, this.uniforms.maxPixelRadius.value = B.maxPixelRadius, this.uniforms.minAlpha.value = B.minAlpha, this.uniforms.enable2DGS.value = B.enable2DGS, this.uniforms.preBlurAmount.value = B.preBlurAmount, this.uniforms.blurAmount.value = B.blurAmount, this.uniforms.clipXY.value = B.clipXY, this.uniforms.focalAdjustment.value = B.focalAdjustment;
    const w = i === null ? A.outputColorSpace : D ? i.texture.colorSpace : o.ColorManagement.workingColorSpace;
    this.uniforms.encodeLinear.value = w !== o.SRGBColorSpace, this.uniforms.ordering.value = B.orderingTexture ?? F.emptyOrdering;
    const G = y.getTextures();
    this.uniforms.splats.value = G[0], this.uniforms.splats2.value = G[1], this.uniforms.time.value = y.time, this.uniforms.deltaTime.value = y.deltaTime, this.uniforms.debugFlag.value = performance.now() / 1e3 % 2 < 1, B.dirty = !1;
  }
  clearSplats() {
  ...
  updateInternal(A) {
    ...
    const I = this.queuedUpdate;
    return this.queuedUpdate = {
      scene: A.scene,
      camera: A.camera,
      // A queued explicit update must not be weakened by a later automatic one.
      autoUpdate: A.autoUpdate && ((I == null ? void 0 : I.autoUpdate) ?? !0)
    }, this.updateRunning || (this.updateRunning = !0, this.updatePromise = this.drainUpdates()), this.updatePromise;
  }
  async drainUpdates() {
    try {
      for (; this.queuedUpdate; ) {
        const A = this.queuedUpdate;
        this.queuedUpdate = null, await this.performUpdate(A);
      }
    } catch (A) {
      throw this.queuedUpdate = null, A;
    } finally {
      this.updateRunning = !1;
    }
  }
  async performUpdate({ scene: A, camera: I, autoUpdate: g }) {
    const B = this.renderer;
    this.ownsTimer && this.timer.update();
    const Q = I.getWorldPosition(new o.Vector3()), E = I.getWorldDirection(new o.Vector3()), i = Q.distanceTo(this.sortedCenter) > 1e-3 * _I(I) || E.dot(this.sortedDir) < 0.999 || this.sortRadial !== this.sortedRadial, D = this.takeAccumulator();
    if (D === this.current)
      throw new Error(
    ...
    try {
      ...
      throw this.releaseAccumulator(D), w;
    }
    const { version: a, sortUpdated: y, generate: c } = s;
    let e = !0;
    const t = i || a !== this.current.version, h = i || y;
    if (g && !t && (e = !1), !e)
      this.releaseAccumulator(D);
    else {
      try {
        c();
      } catch (w) {
      ...
      y && (this.sortCentersRevision += 1), this.display.mappingVersion === D.mappingVersion && !h ? (this.releaseAccumulator(this.display), this.display = D) : this.display !== this.current && this.releaseAccumulator(this.current), this.current = D, this.sortDirty || (this.sortDirty = h), this.setDirty();
    }
  ...
  async driveSort() {
    ...
    if (A < I && (await new Promise((Q) => setTimeout(Q, I - A)), this.disposed))
      return;
    this.sorting = !0, this.sortDirty = !1, this.lastSortTime = performance.now();
    const g = this.current, B = this.activeSplats;
    try {
      const { numSplats: Q, maxSplats: E } = g, i = Math.max(1, Math.ceil(E / 16384)), D = i * 16384;
      this.maxSplats = Math.max(this.maxSplats, D);
      const s = new Uint32Array(this.maxSplats);
      this.sortWorker || (this.sortWorker = new EA());
      const a = this.sortCentersRevision;
      if (this.uploadedSortCentersRevision !== a) {
        const { payload: t, commit: h } = this.sortCenterCache.prepare(g);
        await this.sortWorker.call("setSortCenterState", t), h(), this.uploadedSortCentersRevision = a;
      }
      const y = this.sortRadial, c = await this.sortWorker.call("sortCenters32", {
        numSplats: Q,
        cameraPosition: [
          g.viewOrigin.x,
          g.viewOrigin.y,
      ...
      this.activeSplats = c.activeSplats;
      const e = Math.ceil(c.activeSplats / 16384);
      if (this.orderingTexture && i > this.orderingTexture.image.height && (this.orderingTexture.dispose(), this.orderingTexture = null), this.orderingTexture) {
        const t = this.renderer;
        t.properties.has(this.orderingTexture) ? e > 0 && bA(
          t,
          this.orderingTexture,
          4096,
          e,
          c.ordering
        ) : (this.orderingTexture.image.data = c.ordering, this.orderingTexture.needsUpdate = !0);
      } else {
        const t = new o.DataTexture(
          c.ordering,
          4096,
          i,
          o.RGBAIntegerFormat,
        ...
        t.internalFormat = "RGBA32UI", t.needsUpdate = !0, this.orderingTexture = t;
      }
      this.sortedCenter.copy(g.viewOrigin), this.sortedDir.copy(g.viewDirection), this.sortedRadial = y, this.current === g && this.display !== g && (this.releaseAccumulator(this.display), this.display = g), this.setDirty();
    } catch (Q) {
      if (this.disposed) return;
      throw this.sortDirty = !0, this.current === g && g !== this.display && this.accumulators.length === 0 && (this.current = this.display, this.releaseAccumulator(g), this.activeSplats = B, this.sortDirty = !1, this.uploadedSortCentersRevision = -1), Q;
    } finally {
      this.sorting = !1;
    }
  }
  render(A, I) {
    const g = F.gaussianSplatOverride;
    try {
      F.gaussianSplatOverride = this, this.renderer.render(A, I);
    } finally {
      F.gaussianSplatOverride = g;
    }
  }
  renderTarget({
    scene: A,
    ...
    const g = this.backTarget ?? this.target;
    if (!g)
      throw new Error("No target");
    const B = this.renderer.getRenderTarget(), Q = F.gaussianSplatOverride;
    try {
      this.renderer.setRenderTarget(g), F.gaussianSplatOverride = this, this.renderer.render(A, I);
    } finally {
      F.gaussianSplatOverride = Q, this.renderer.setRenderTarget(B);
    }
    return g !== this.target && ([this.target, this.backTarget] = [this.backTarget, this.target]), g;
  }
  ...
  async readTarget() {
    ...
    (!this.superPixels || this.superPixels.length < g) && (this.superPixels = new Uint8Array(g));
    ...
    if (Q === 1)
      return B;
    const E = A / Q, i = I / Q, D = E * i * 4;
    (!this.targetPixels || this.targetPixels.length < D) && (this.targetPixels = new Uint8Array(D));
    const s = this.targetPixels, a = Q * Q;
    for (let y = 0; y < i; y++) {
      const c = y * E;
      for (let e = 0; e < E; e++) {
        const t = e * Q;
        ...
        for (let S = 0; S < Q; S++) {
          ...
          for (let k = 0; k < Q; k++) {
            const x = (J + t + k) * 4;
            h += B[x], w += B[x + 1], G += B[x + 2], N += B[x + 3];
          }
        }
        const R = (c + e) * 4;
        s[R] = h / a, s[R + 1] = w / a, s[R + 2] = G / a, s[R + 3] = N / a;
      }
    }
    return s;
  }
  ...
  async renderCubeMap({
    ...
    if (!F.cubeRender || F.cubeRender.target.width !== g || F.cubeRender.near !== B || F.cubeRender.far !== Q) {
      F.cubeRender && F.cubeRender.target.dispose();
      const e = new o.WebGLCubeRenderTarget(g, {
        format: o.RGBAFormat,
        type: o.UnsignedByteType,
        generateMipmaps: D,
        minFilter: D ? o.LinearMipMapLinearFilter : o.LinearFilter,
        magFilter: o.LinearFilter,
        colorSpace: D ? o.LinearSRGBColorSpace : o.SRGBColorSpace
      }), t = new o.CubeCamera(B, Q, e);
      F.cubeRender = { target: e, cubeCamera: t, near: B, far: Q };
    }
    const { target: s, cubeCamera: a } = F.cubeRender;
    a.position.copy(I);
    const y = /* @__PURE__ */ new Map();
    for (const e of E)
      y.has(e) || y.set(e, e.visible), e.visible = !1;
    const c = F.gaussianSplatOverride;
    try {
      if (i) {
        const e = new o.Camera();
        e.position.copy(I), await this.update({ scene: A, camera: e });
      }
      return F.gaussianSplatOverride = this, a.update(this.renderer, A), s.texture;
    } finally {
      F.gaussianSplatOverride = c;
  ...
  async readCubeTargets() {
    ...
    const { target: A } = F.cubeRender, { width: I, height: g } = A, B = [], Q = [];
    for (let E = 0; E < A.texture.images.length; ++E) {
      const i = I * g * 4, D = new Uint8Array(i);
      Q.push(D);
      const s = this.renderer.readRenderTargetPixelsAsync(
        A,
        0,
        0,
        I,
  ...
  async renderEnvMap({
    ...
    var s;
    const D = await this.renderCubeMap({
      scene: A,
      worldCenter: I,
      size: g,
      near: B,
    ...
    return F.pmrem || (F.pmrem = new o.PMREMGenerator(this.renderer)), (s = F.pmrem) == null ? void 0 : s.fromCubemap(D).texture;
  }
  // Utility function to recursively set the envMap property for any
  // THREE.MeshStandardMaterial within the subtree of root.
  recurseSetEnvMap(A, I) {
    A.traverse((g) => {
      if (g instanceof o.Mesh)
        if (Array.isArray(g.material))
          for (const B of g.material)
            B instanceof o.MeshStandardMaterial && (B.envMap = I);
        else
          g.material instanceof o.MeshStandardMaterial && (g.material.envMap = I);
    });
  }
...
F.emptyOrdering = (() => {
  const I = new Uint32Array(16384), g = new o.DataTexture(I, 4096, 1);
  return g.format = o.RGBAIntegerFormat, g.type = o.UnsignedIntType, g.internalFormat = "RGBA32UI", g.needsUpdate = !0, g;
})(), F.cubeRender = null, F.pmrem = null;
let rA = F;
function $I(C) {
  return C == null ? void 0 : C.isXRRenderTarget;
}
export {
  rA as GaussianSplatRenderer,
  U as SplatAccumulator,
  BA as SplatEdit,
  $A as SplatEditRgbaBlendMode,
  AI as SplatEditSdf,
  _A as SplatEditSdfType,
  p as SplatEdits,
  YA as SplatFileType,
  xI as SplatLoader,
  gA as SplatMesh,
