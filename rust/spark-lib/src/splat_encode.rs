...
pub fn encode_packed_splat(
    ...
    let u_a = float_to_u8(opacity, 0.0, 1.0);
...
pub fn encode_sh2_internal(sh2: &[f32], sh2_scale: f32) -> [u32; 4] {
    let bytes: [u8; 15] =
        array::from_fn(|i| (sh2[i] * sh2_scale).clamp(-127.0, 127.0).round() as i8 as u8);
    [
        (bytes[0] as u32)
            | ((bytes[1] as u32) << 8)
            | ((bytes[2] as u32) << 16)
