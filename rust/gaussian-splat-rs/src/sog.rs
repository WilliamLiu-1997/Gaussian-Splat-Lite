//! Ordinary SOG decoding. Only the current property group and a packed SH palette
//! stay in WASM; completed properties live directly in the transferable JS arrays.
use std::{array, io::Cursor};

use anyhow::{anyhow, bail, ensure, Context, Result};
use gaussian_splat_lib::{
    decoder::{SplatInit, SplatReceiver},
    splat_encode::{encode_splat_sh_rgb, get_splat_tex_size},
};
...
impl SogDecodeSession {
    ...
    pub fn new(metadata: &str) -> Result<SogDecodeSession, JsValue> {
        let meta = Metadata::parse(metadata).map_err(js_error)?;
        ...
        if session.meta.degree != 0 || session.meta.groups[0].0 != "centroids" {
            session.init_splats().map_err(js_error)?;
        }
        Ok(session)
    }

    pub fn plan(&self) -> String {
        let files: Vec<_> = self.meta.groups.iter().map(|(_, files)| files).collect();
        serde_json::to_string(&files).expect("asset names are serializable")
    }
    ...
    pub fn decode_asset(
        ...
        self.load_image(chunks, method, uncompressed_size, crc)
            .map_err(js_error)
    }

    pub fn decode_batch(&mut self) -> Result<bool, JsValue> {
        self.decode_group_batch().map_err(js_error)
    }

    pub fn finish(mut self) -> Result<Object, JsValue> {
        if self.group != self.meta.groups.len() {
            return Err(js_error("decode is incomplete"));
        }
        let mut splats = self
            .splats
            .take()
            .ok_or_else(|| js_error("missing output"))?;
        splats.finish().map_err(js_error)?;
        let output = splats.into_splat_object();
...
impl SogDecodeSession {
    ...
    fn decode_group_batch(&mut self) -> Result<bool> {
        ensure!(
            self.group < self.meta.groups.len()
                && self.images.len() == self.meta.groups[self.group].1.len(),
        ...
        match role {
            ...
            "labels" => {
                let words = SH_WORDS[self.meta.degree];
                self.words.resize(count * 4, 0);
                for (block, output) in [&splats.sh1, &splats.sh2, &splats.sh3a, &splats.sh3b]
                    .into_iter()
                    .enumerate()
                {
                    let Some(output) = output else { continue };
                    for i in 0..count {
                        let label = image.channel(self.base + i, 0) as usize
                            | ((image.channel(self.base + i, 1) as usize) << 8);
                        ensure!(
                            label < self.meta.palette_count,
                            "SH label exceeds palette at splat {}",
                            self.base + i
                        );
                        let offset = label * words + block * 4;
                        self.words[i * 4..i * 4 + 4]
                            .copy_from_slice(&self.palette[offset..offset + 4]);
                    }
                    output
                        .subarray((self.base * 4) as u32, ((self.base + count) * 4) as u32)
                        .copy_from(&self.words);
                }
            }
