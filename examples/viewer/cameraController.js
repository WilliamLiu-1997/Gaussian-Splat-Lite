...
class CameraController extends EventDispatcher {
  ...
  constructor(renderer, scene, camera, options = {}) {
    ...
    this.#scene = scene;
    this.#camera = camera;
    this.#domElement = controllerOptions.domElement ?? renderer.domElement;
    this.damping = controllerOptions.damping ?? 0.15;
    ...
    this.#pointerDownFilter =
      typeof controllerOptions.pointerDownFilter === "function"
        ? controllerOptions.pointerDownFilter
        : null;
    this.#raycastHitFilter =
      typeof controllerOptions.raycastHitFilter === "function"
        ? controllerOptions.raycastHitFilter
        : null;
    this.init();
  }
  get enabled() {
  ...
  #getPositionUpDirection(position, target) {
    ...
    return this.#getWorldUp(target);
  }
  #getWorldUp(target) {
    target.copy(this.#camera.up);
    if (target.lengthSq() <= THRESHOLD * THRESHOLD) {
      target.set(0, 1, 0);
    }
  ...
  #rotateNearAnchor(rotateVec) {
    ...
    this.#camera.getWorldDirection(_forward);
    const worldUp = this.#getWorldUp(_positionUp);
  ...
  #keepCameraUp(anchorPoint, useExteriorZoomAlignment = false) {
    ...
    // the camera's configured world-up direction.
